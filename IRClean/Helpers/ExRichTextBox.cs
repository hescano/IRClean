...
public class ExRichTextBox : System.Windows.Forms.RichTextBox
{
    ...
    private string GetImagePrefix(Image _image)
    {
        ...
        int picw = Convert.ToInt32(Math.Round((_image.Width / xDpi) * HMM_PER_INCH));

        // Calculate the current height of the image in (0.01)mm
        int pich = Convert.ToInt32(Math.Round((_image.Height / yDpi) * HMM_PER_INCH));

        // Calculate the target width of the image in twips
        int picwgoal = Convert.ToInt32(Math.Round((_image.Width / xDpi) * TWIPS_PER_INCH));

        // Calculate the target height of the image in twips
        int pichgoal = Convert.ToInt32(Math.Round((_image.Height / yDpi) * TWIPS_PER_INCH));
    ...
    {
        return _originalRtf.Replace(new string('\0', 255), "");
    }
